...
class Instruction(object):
    ...
    def __init__(self, pc=None, expression=None, env=None):
        ...
        if pc:
            add_edge(pc, self)
        self.expression = Expr(expression) if expression else Expr(u"")
        self.env = env
        self.line = None
    ...
    def set_line(self, cur):
        self.line = count(Instruction.program_text[:cur], u'\n') + 1
...
class Graph(object):
    ...
    def parse_lines(self, program, pc, env, ctn_inst, brk_inst, ret_inst):
        while program:
            ...
            elif program.front_match(VALIDATE_REGEX):
                # ignore validate call
                program.forward(-1)
                program.front_jump(u';')
                program.forward(1)
    ...
    def __init__(self, program):
        ...
        while program:
            ...
            if program.front_match(INCLUDE_REGEX):
            ...
            elif program.text[program.front_jump(u';{')] == u';':
                ...
                if re.search(u'validate', program.group()):
                    # ignore validate function
                    program.forward(1)
                    program.front_jump()
                    program.forward(1)
                    continue
